...
export function generateInspectionPDF(
  ...
  doc.setFontSize(10.5);
  doc.setTextColor(darkNavy);
  doc.text("5. CRITÈRES DE DIAGNOSTIC ET MÉTHODOLOGIE D'ANALYSE", 15, y);
  doc.setDrawColor(lightBlue);
  doc.setLineWidth(0.35);
  doc.line(15, y + 1.5, 125, y + 1.5);
  ...
  const descriptionParts = [
    ...
    techOptions.length > 0 ? `Options techniques sélectionnées : ${techOptions.join(" | ")}` : "",
    maintenanceOpts.length > 0 ? `Options de maintenance sélectionnées : ${maintenanceOpts.join(" | ")}` : ""
  ].filter(Boolean);
